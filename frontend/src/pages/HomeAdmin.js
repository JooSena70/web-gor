...
const HomeAdmin = () => {
  ...
  const getBookingAtHour = (fieldId, hour) => {
    return filteredBookings.find(b => b.field_id === fieldId && isHourBooked(b, hour));
  };
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="field">
        <label htmlFor="label"><strong>Select a Date:</strong></label>
        <input
          id="dateInput"
          type="date"
          className="input"
                ...
                {(() => {
                  ...
                  while (hour < 24) {
                    ...
                    if (booking) {
                      ...
                      const alreadyRendered = tds.find(td => td.key === `booking-${booking.id}`);
                      if (alreadyRendered) {
                        hour++; // skip this hour
                        continue;
                      }
                      ...
                      tds.push(
                        <td key={`empty-${hour}`} style={{ textAlign: 'center' }}>
                          {/* empty */}
                        </td>
                      );
                      hour++;
      ...
      {selectedBooking && (
        <div className="modal-overlay" onClick={() => setSelectedBooking(null)}>
          ...
          >
            ...
            <p><strong>Field:</strong> {getFieldName(selectedBooking.field_id)}</p>
            <p><strong>Date:</strong> {selectedBooking.booking_date}</p>
            <p><strong>Time:</strong> {selectedBooking.start_time.slice(0, 5)} - {selectedBooking.end_time.slice(0, 5)}</p>
