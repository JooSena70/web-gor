        ...
        <tbody>
          {payments.map((payment, index) => (
            <tr key={payment.id}>
              <td>{index + 1}</td>
              <td>
                #{payment.booking_id} - {bookings.find(b => b.id === payment.booking_id)?.booking_date || '-'}
              </td>
              <td>Rp {parseFloat(payment.amount).toLocaleString("id-ID", { minimumFractionDigits: 2 })}</td>
              <td>{payment.payment_date}</td>
              <td>{payment.method}</td>
              <td>{payment.status}</td>
              <td>
                <button onClick={() => editPayment(payment)} className="button is-small is-info mr-2">Edit</button>
          ...
          {payments.length === 0 && (
            <tr>
              <td colSpan="7" className="has-text-centered">No payments available</td>
            </tr>
          )}
        </tbody>
      </table>
